...
function updateStats() {
    ...
    const adminCount = allUsers.filter(u =>
        u.role === 'admin' || u.role_name === 'admin'
    ).length;
...
function renderUsers() {
    ...
    pageUsers.forEach(user => {
        ...
        const roleDisplayName = user.role_display_name || getRoleDisplayName(roleName);
...
async function handleUserFormSubmit(e) {
    ...
    saveBtn.innerHTML = '<span class="loading-spinner small"></span> Збереження...';
    ...
    try {
        ...
        if (data.success) {
            showToast(userId ? 'Користувача оновлено' : 'Користувача створено', 'success');
            closeUserModal();
            await loadUsers();
        } else {
