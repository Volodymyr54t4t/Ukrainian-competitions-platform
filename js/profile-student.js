...
function renderInstitutionsSelect() {
  ...
  if (institutionsList.length === 0) {
    select.innerHTML =
      '<option value="">Немає закладів для цього міста</option>';
...
function renderProfile() {
  ...
  document.getElementById("infoInstitution").textContent =
...
function renderInterests() {
  ...
  if (interests.length === 0) {
    container.innerHTML =
      '<span class="empty-text">Предмети інтересу не вказані</span>';
    return;
  }
...
function renderEditAchievements() {
  ...
  container.innerHTML = editAchievements
    .map(
      (ach, index) => `
...
function renderEditCertificates() {
  ...
  container.innerHTML = editCertificates
    .map(
      (cert, index) => `
        <div class="dynamic-item" data-index="${index}">
            <div class="dynamic-item-header">
...
async function saveProfile(e) {
  ...
  saveBtn.innerHTML =
    '<div class="loading-spinner" style="width:18px;height:18px;"></div> Збереження...';

  // Збір даних форми
  const citySelect = document.getElementById("editCity");
  const selectedCityOption = citySelect.options[citySelect.selectedIndex];
  const cityName =
    selectedCityOption && selectedCityOption.dataset.name
      ? selectedCityOption.dataset.name
      : null;
